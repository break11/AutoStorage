...
class CNetCmd:
    def __init__(self, ClientID=None, Event=None, Obj_UID=None, PropName=None, PropValue=None, ExtCmdData=None ):
    ...
    def toString( self, bDebug = False ):
        ...
        elif self.Event <= EV.ObjPropUpdated:     return f"{self.ClientID}:{cmd}:{self.Obj_UID}:{self.sPropName}:{ CStrTypeConverter.ValToStr(self.PropValue) }"
    ...
    def fromString( sVal ):
        ...
        if ev <= EV.ClientDisconnected:
            ...
            if ev <= EV.ObjPrepareDelete:
                ...
                propName  = l[3]
                propValue = CStrTypeConverter.ValFromStr( l[4] )
                return CNetCmd( ClientID=Client_UID, Event=ev, Obj_UID=objUID, PropName=propName, PropValue=propValue )
