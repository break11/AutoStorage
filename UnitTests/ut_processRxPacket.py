...
class Test_processRxPacket(unittest.TestCase):
    ...
    def test_CA_after_HW(self):
        ...
        _processRxPacket( cmd, ACC_cmd, TX_FIFO, lastTXpacketN=0, processAcceptedPacket=None )
    ...
    def test_CA_Normal_and_Duplicate_in_WorkLoop(self):
        ...
        _processRxPacket( cmd, ACC_cmd, TX_FIFO, lastTXpacketN=1, processAcceptedPacket=None )
        ...
        cmd.packetN = 20
        _processRxPacket( cmd, ACC_cmd, TX_FIFO, lastTXpacketN=21, processAcceptedPacket=None )
        self.assertEqual( cmd.status, EPacket_Status.Duplicate )

        # проверка на то, что пакет отстающий больше, чем на 1 - распознается как ошибка
        cmd.packetN = 20 # рядовой случай
        _processRxPacket( cmd, ACC_cmd, TX_FIFO, lastTXpacketN=22, processAcceptedPacket=None )
        ...
        _processRxPacket( cmd, ACC_cmd, TX_FIFO, lastTXpacketN=20, processAcceptedPacket=None )
        self.assertEqual( cmd.status, EPacket_Status.Error )
        cmd.packetN = 22 # рядовой случай
        _processRxPacket( cmd, ACC_cmd, TX_FIFO, lastTXpacketN=20, processAcceptedPacket=None )
        self.assertEqual( cmd.status, EPacket_Status.Error )
        cmd.packetN = 0 # случай с переходом
        _processRxPacket( cmd, ACC_cmd, TX_FIFO, lastTXpacketN=999, processAcceptedPacket=None )
        self.assertEqual( cmd.status, EPacket_Status.Error )
