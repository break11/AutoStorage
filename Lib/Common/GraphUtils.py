...
def nodeProp(nxGraph, nodeID, sPropName, genError=True):
    try:
        return nxGraph.nodes()[ nodeID ][ sPropName ]
    except KeyError:
        if genError: print( f"{SC.sError} Node {nodeID} has not prop with name {sPropName}!" )
        return None
...
def getAgentAngle(nxGraph, tEdgeKey, agent_angle):
    ...
    if AgentEdgeAngle <= math.pi/2.4 :
        return math.degrees( r_vec.selfAngle() ), False
    elif AgentEdgeAngle >= (math.pi - math.pi/2.4):
...
def getAgentSide(nxGraph, tEdgeKey, agent_angle):
    if not tEdgeKey: return SGT.ESide.Default
